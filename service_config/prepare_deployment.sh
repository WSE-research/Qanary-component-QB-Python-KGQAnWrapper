...
if [ -z "$SPRING_BOOT_ADMIN_USERNAME" ]; then
  ...
  sed -i "s/SPRING_BOOT_ADMIN_USERNAME/$SPRING_BOOT_ADMIN_USERNAME/g" ./service_config/files/qb-kgqanwrapper-dbpedia
  sed -i "s/SPRING_BOOT_ADMIN_USERNAME/$SPRING_BOOT_ADMIN_USERNAME/g" ./service_config/files/qb-kgqanwrapper-wikidata
fi
